// 執行 ./rbi 7 11 1 99 22 33 44 9 3 5
#include <stdio.h>
#include <string.h>
 ...
 * tree.
 *
 * Maximum number of bytes in a process: 1 << (sizeof(void *) << 3). //  8 bytes * 8 bits = 64 bits
...
#define rb_gen_insert(x_attr, x_prefix, x_rbt_type, x_type, x_field, x_less)  \
    ...
    {                                                                         \
        ...
        rbt_node_new(x_type, x_field, rbtree, node);  /* 節點為紅色  */    \
        /* Wind. */                                                           \
        path->node = rbtree->root;                                            \
        for (pathp = path; pathp->node; pathp++) {   /* 往後找到要插入的葉子 */                         \
        ...
        /* Unwind */                                                         \
        while (pathp-- != path) {    /* pathp:父節點 pathp[1]:子節點*/                                         \
...
static tree_t tree;
static node_t nodes[1 << LOG2_MAX_NODES]; // 64
...
{
    tree_new(&tree); // tree->root = NULL
    node_t *nodei = nodes;
    for (++argv; *argv; ++argv, ++nodei) {
        int v, n;
        if ((char *) nodei == (char *) nodes + sizeof(nodes)) { // excess nodes[64]
